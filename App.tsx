      ...
      >
        ...
        <div className="hidden md:flex items-center gap-12">
          <button 
            ...
            className={`nav-link text-[10px] font-bold uppercase transition-colors ${currentView === 'about' ? 'text-[#C5A059] opacity-100' : 'text-[#1a1a1a] opacity-70 hover:opacity-100'}`}
          >
            Origins
          </button>
          ...
          <div className="relative" onMouseEnter={() => setIsArchHovered(true)}>
            <button 
              ...
              className={`nav-link text-[10px] font-bold uppercase transition-colors flex items-center gap-2 ${currentView === 'architecture' || currentView.startsWith('pillar') ? 'text-[#C5A059] opacity-100' : 'text-[#1a1a1a] opacity-70 hover:opacity-100'}`}
            >
            ...
            <AnimatePresence>
              {isArchHovered && (
                <motion.div 
                  ...
                  className="absolute top-full -left-20 mt-4 w-[600px] bg-[#FFF2EC] border border-black/5 shadow-2xl p-8 grid grid-cols-3 gap-8 z-[500]"
                >
                      ...
                      <div className="space-y-2.5">
                        {system.items.map((item) => (
                          <button 
                            ...
                            className="block text-left font-serif text-[1.1rem] leading-tight text-[#1a1a1a] hover:text-[#C5A059] transition-colors tracking-wide"
                          >
                            {item.name}
                          </button>
                        ))}
          ...
          <a 
            ...
            target="_blank" 
            className="text-xs font-bold uppercase tracking-[0.25em] text-[#E21E3F] border-b border-[#E21E3F] pb-0.5 hover:border-b-2 hover:pb-1 transition-all duration-300 ease-[cubic-bezier(0.23,1,0.32,1)]"
          >
            Audit My System
          </a>
        </div>
